...
static void test_basic_api()
{
...

    {
        ...
        stream.template peek_bytes<endian::u8>(first_peek);
        stream.template peek_bytes<endian::u8>(second_peek);
        stream.template read_bytes<endian::u8>(first_read);
        EXPECT_EQ(first_peek, second_peek);
        EXPECT_EQ(first_peek, first_read);

        uint8_t second_read = 0;
        uint8_t third_peek = 0;
        stream.template peek_bytes<endian::u8>(third_peek);
        stream.template read_bytes<endian::u8>(second_read);
...

    {
        ...
        uint8_t first_read = 0;
        stream.template peek_bytes<endian::u8>(first_peek_at_3, 2);
        stream.template peek_bytes<endian::u8>(second_peek_at_1, 0);
        stream.template read_bytes<endian::u8>(first_read);
