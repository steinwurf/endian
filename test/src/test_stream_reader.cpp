...
static void test_basic_api()
{
...

    {
        ...
        stream.template peek<1>(first_peek);
        stream.template peek<1>(second_peek);
        stream.template read<1>(first_read);
        EXPECT_EQ(first_peek, second_peek);
        EXPECT_EQ(first_peek, first_read);

        uint8_t second_read = 0;
        uint8_t third_peek = 0;
        stream.template peek<1>(third_peek);
        stream.template read<1>(second_read);
...

    {
        ...
        uint8_t first_read = 0;
        stream.template peek<1>(first_peek_at_3, 2);
        stream.template peek<1>(second_peek_at_1, 0);
        stream.template read<1>(first_read);
