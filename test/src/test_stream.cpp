...
void write_and_read_value_type(ValueType min, ValueType max)
{
    ...
    {
        stream_writer.template write<Bytes>(max);
    }
    ...
    {
        ValueType value = 0;
        stream_reader.template read<Bytes>(value);
        EXPECT_EQ(max, value);
    }
    ...
    for (uint32_t i = 0; i < elements; i++)
    {
        stream_writer.template write<Bytes>(min);
    }
...
void write_and_read_random_value_type(ValueType min, ValueType max)
{
    ...
    for (uint32_t i = 0; i < elements; i++)
    {
        ...
        stream_writer.template write<Bytes>(values[i]);
    }
    for (uint32_t i = 0; i < elements; i++)
    {
        ValueType value;
...
void run_write_peek_and_read_variadic_bytes()
{
    ...
    for (uint32_t i = 0; i < elements; i++)
    {
        ...
        case 0:
        {
            ...
            writer.template write<1>((uint8_t)values[i]);
            break;
        }
        case 1:
        {
            ...
            writer.template write<2>((uint16_t)values[i]);
            break;
        }
        case 2:
        {
            ...
            writer.template write<3>((uint32_t)values[i]);
            break;
        }
        case 3:
        {
            ...
            values[i] = (uint32_t)distribution(engine);
            writer.template write<4>((uint32_t)values[i]);
            break;
        }
        case 4:
        {
            ...
            writer.template write<5>((uint64_t)values[i]);
            break;
        }
        case 5:
        {
            ...
            values[i] = (uint64_t)distribution(engine);
            writer.template write<6>((uint64_t)values[i]);
            break;
        }
        case 6:
        {
            ...
            values[i] = (uint64_t)distribution(engine);
            writer.template write<7>((uint64_t)values[i]);
            break;
        }
        case 7:
        {
            ...
            values[i] = (uint64_t)distribution(engine);
            writer.template write<8>((uint64_t)values[i]);
            break;
        }
        }
    }
    ...
    for (uint32_t i = 0; i < elements; i++)
    {
        ...
        case 0:
            // test peek
            reader.template peek<1>(last_uint8_t);
            EXPECT_EQ(values[i], last_uint8_t);

            // test read
            last_uint8_t = 0;
            reader.template read<1>(last_uint8_t);
            EXPECT_EQ(values[i], last_uint8_t);
            break;
        case 1:
            // test peek
            reader.template peek<2>(last_uint16_t);
            EXPECT_EQ(values[i], last_uint16_t);

            // test read
            last_uint16_t = 0;
            reader.template read<2>(last_uint16_t);
            EXPECT_EQ(values[i], last_uint16_t);
            break;
        case 2:
            // test peek
            reader.template peek<3>(last_uint24_t);
            EXPECT_EQ(values[i], last_uint24_t);

            // test read
            last_uint24_t = 0;
            reader.template read<3>(last_uint24_t);
            EXPECT_EQ(values[i], last_uint24_t);
            break;
        case 3:
            // test peek
            reader.template peek<4>(last_uint32_t);
            EXPECT_EQ(values[i], last_uint32_t);

            // test read
            last_uint32_t = 0;
            reader.template read<4>(last_uint32_t);
            EXPECT_EQ(values[i], last_uint32_t);
            break;
        case 4:
            // test peek
            reader.template peek<5>(last_uint40_t);
            EXPECT_EQ(values[i], last_uint40_t);

            // test read
            last_uint40_t = 0;
            reader.template read<5>(last_uint40_t);
            EXPECT_EQ(values[i], last_uint40_t);
            break;
        case 5:
            // test peek
            reader.template peek<6>(last_uint48_t);
            EXPECT_EQ(values[i], last_uint48_t);

            // test read
            last_uint48_t = 0;
            reader.template read<6>(last_uint48_t);
            EXPECT_EQ(values[i], last_uint48_t);
            break;
        case 6:
            // test peek
            reader.template peek<7>(last_uint56_t);
            EXPECT_EQ(values[i], last_uint56_t);

            // test read
            last_uint56_t = 0;
            reader.template read<7>(last_uint56_t);
            EXPECT_EQ(values[i], last_uint56_t);
            break;
        case 7:
            // test peek
            reader.template peek<8>(last_uint64_t);
            EXPECT_EQ(values[i], last_uint64_t);

            // test read
            last_uint64_t = 0;
            reader.template read<8>(last_uint64_t);
            EXPECT_EQ(values[i], last_uint64_t);
            break;
        }
    }
...
static void run_write_and_read_string_test()
{
    ...
    writer.template write<2>((uint16_t)first.size());
    writer.write((uint8_t*)first.c_str(), first.size());
    writer.template write<2>((uint16_t)second.size());
    writer.write((uint8_t*)second.c_str(), second.size());
    writer.template write<2>((uint16_t)third.size());
